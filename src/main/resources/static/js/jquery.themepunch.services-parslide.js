...
(function ($, undefined) {
        ...
        if (!top.hasClass('DuringAnimation')) {
            ...
            if (opt.actSlide >= 0) {
                ...
                for (var i = 0; i < slidearray.length; i++) {
                    ...
                    if (nextitem == null && _readmore.length > 0) nextitem = top.parent().find('ul:first >li').eq(actindex).find('.slide');
                }
        ...
        top.find('ul:first >li .slide').each(function (i) {
            ...
            if (direction == 0) {
                ...
                if ($this.data("id") == opt.maxitem - 1) {
                    ...
                    $this.animate({'left': (1 * minusslide) + "px"}, {duration: 300, queue: false});
                } else {

                    // CHECK IF THE SLIDE IS ALREADY OUTSIDE LEFT OF THE WINDOW?
                    if ($this.position().left < 0) {
                        $this.animate({'left': overslide + "px"}, {duration: 0, queue: false});
                    } else {

                        setTimeout(function () {

                            $this.animate({'left': opt.positionArray[$this.data('id')] + "px"}, {
                                duration: 300,
                                queue: false
                            });
                        }, 0);//+$this.data('id')*10);
                ...
                if ($this.data("id") == opt.slideAmount) {
                    $this.animate({'left': overslide + "px"}, {duration: 300, queue: false});
                } else {

                    // CHECK IF THE SLIDE IS ALREADY OUTSIDE LEFT OF THE WINDOW?
                    if ($this.position().left >= opt.positionArray[opt.maxitem - 1]) {
                        $this.css({'left': minusslide + "px"});
                    ...
                    }, 0);//+(opt.slideAmount+1-$this.data('id'))*10);
    ...
    function prepareSlides(top, opt) {
        ...
        top.find('ul:first').wrap('<div style="position:absolute;top:0;left:0" class="services-wrapper-top"></div>');
