...
(function ($, undefined) {
            ...
            return this.each(function () {
                ...
                if (opt.touchenabled == "on")
                    top_container.swipe({
                        ...
                        swipeRight: function () {
                            ...
                            leftClick(top_container, opt);


                        },
                        excludedElements: ".close, .morebutton, .closer, .kenburn-video-overlay, .kenburn-video-button, .hover-more-sign, .hover-blog-link-sign .thumbnails, .closebutton",
                ...
                if (opt.mouseWheel == "on") {
                    top_container.bind('mousewheel', function (event, delta) {
                        ...
                        if (delta < 0)
                            top.data('mw', setTimeout(function () {
                                rightClick(top_container, opt);
                            }, 100));
                        else
                            top.data('mw', setTimeout(function () {
    ...
    ///////////////////////////////
    // THE RIGHT SLIDE FUNCTION //
    //////////////////////////////
    function rightClick(top, opt) {
        ...
        if (!top.hasClass('DuringAnimation')) {
            ...
            setTimeout(function () {
                top.removeClass('DuringAnimation')
            }, 500);
            ...
            if (opt.actSlide >= 0) {
                ...
                for (var i = 0; i < slidearray.length; i++) {
                    actindex++;
                    if (actindex == slidearray.length)
                        actindex = 0;
                    var _readmore = top.parent().find('ul:first >li').eq(actindex).find('.slide').data('pagemore');
                    if (nextitem == null && _readmore.length > 0)
                        nextitem = top.parent().find('ul:first >li').eq(actindex).find('.slide');
                }
        ...
        if (!top.hasClass('DuringAnimation')) {
            ...
            if (opt.maxitem > opt.slideAmount) {
                if ((parseInt(top.find('ul:first >li .slide:first-child').data('id'), 0) != 0 && opt.carousel == "off") || (opt.carousel != "off")) {
                    ...
                    if (parseInt(top.find('ul:first >li .slide:first-child').data('id'), 0) == 0 && opt.carousel == "off")
    ...
    function openReadMorePart(slide, opt, dir) {
        ...
        opt.top.parent().find('.page-more').remove();
        ...
        var _readmore = opt.top.parent().find('.page-more');
        _readmore.css({'visibility': 'visible'});

        if (dir == 2) {
            _readmore.css({'left': "-150px", "opacity": '0.0'});
            _readmore.animate({'left': '0px', "opacity": '1.0'}, {duration: 500, queue: false});
        } else {
            if (dir == 1) {
                _readmore.css({'left': "150px", "opacity": '0.0'});
                _readmore.animate({'left': '0px', "opacity": '1.0'}, {duration: 500, queue: false});
                ...
                _readmore.css({'top': (0 - (opt.height * 1.5)) + "px"});
                _readmore.animate({'top': '0px'}, {duration: 500, queue: false});
            }
        }
    ...
    function reSlideItems(top, opt, direction) {
        ...
        var maxpos = ((opt.slidewidths + opt.slideSpacing) * (opt.slideAmount + 1));
        opt.transition = 1;
        ...
        top.find('ul:first >li .slide').each(function (i) {
            ...
            if (direction == 0) {
                ...
                if ($this.data("id") == opt.slideAmount) {
                    ...
                    //alert($this.position().left+"  "+opt.positionArray[opt.maxitem-1]);
                    // CHECK IF THE SLIDE IS ALREADY OUTSIDE LEFT OF THE WINDOW?

                    if ($this.position().left + 1 >= opt.positionArray[opt.maxitem - 1]) {
    ...
    function prepareSlidesContainer(top, opt) {
        top.find('ul:first').wrap('<div class="main-container" style="z-index:7;top:0px;left:0px;width:' + opt.width + 'px;height:' + opt.height + 'px;overflow:hidden"><div class="slider_holder" style="position:relative;width:' + opt.width + 'px;height:' + opt.height + 'px;overflow:hidden"></div></div>');
    ...
    function startRotation(item, opt) {
        ...
        setTimeout(function () {
            item.find('.paradigm-preloader').remove();
        }, 300);
